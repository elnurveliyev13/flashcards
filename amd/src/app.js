...
define(['core/str'], function(str) {
  ...
  function init(rootid, baseurl, cmid, instanceid, sesskey) {
    ...
    function applyI18N(){
      ...
      const thFront=$("#thFront"); if(thFront) thFront.textContent=t("listCols").front;
      const thStage=$("#thStage"); if(thStage) thStage.textContent=t("listCols").stage;
      const thActions=$("#thActions"); if(thActions) thActions.textContent=t("listCols").actions||t("listCols").play||"";
    ...
    $("#uImage").addEventListener("change", async e=>{const f=e.target.files?.[0]; if(!f)return;
      const imgNameEls = $$('#imgName');
      imgNameEls.forEach(el => el.textContent = f.name);
      lastImageKey="my-"+Date.now().toString(36)+"-img";
      await idbPut(lastImageKey,f);
      $("#imgPrev").src=URL.createObjectURL(f);
      $("#imgPrev").classList.remove("hidden");
      const imgBadge = $("#imgBadge");
      if(imgBadge) imgBadge.classList.remove("hidden");
    });
    $("#uAudio").addEventListener("change", async e=>{const f=e.target.files?.[0]; if(!f)return;
      const audNameEls = $$('#audName');
      audNameEls.forEach(el => el.textContent = f.name);
    ...
    // Clear buttons for audio and image
    $$('#btnClearAud').forEach(btn => {
      btn.addEventListener("click",()=>{
        $("#uAudio").value="";
        $$('#audName').forEach(el => el.textContent="");
        const audPrev = $("#audPrev");
        if(audPrev) {
          audPrev.classList.add("hidden");
          audPrev.src="";
        }
        const audBadge = $("#audBadge");
        if(audBadge) audBadge.classList.add("hidden");
        lastAudioKey=null;
      });
    });
    $$('#btnClearImg').forEach(btn => {
      btn.addEventListener("click",()=>{
        $("#uImage").value="";
        $$('#imgName').forEach(el => el.textContent="");
    ...
    function buildListRows(){
      ...
      filtered.forEach(async r=>{
        ...
        tr.innerHTML=`<td class="front-cell">${r.front||"-"}</td>
          <td class="stage-cell"><span class="badge">${r.stage}</span></td>
          <td class="actions-cell row" style="gap:6px"></td>`;
        const cells = Array.from(tr.children);
        // Defensive: ensure layout stays 3-column even if markup changes.
        if(cells[1]) cells[1].classList.add("stage-cell");
        if(cells[2]) cells[2].classList.add("actions-cell");
        if(cells.length > 3){
          cells.slice(2, cells.length-1).forEach(td => td.remove());
        }
